"""
Correctness tests for SM90 TP=2 sparse decode kernel (sparse_fp8_swapsab_tp2).

Routes through `flash_mla.flash_mla_with_kvcache` -> SparseAttnDecodeParams ->
Decode_Sm90_Impl::run_, which dispatches h_q=32 to the swap-AB N=32 wgmma path
(P^T = K @ Q^T). Single variant -- no env-var split like TP=8 has, since
swap-AB is the only sensible path for h_q=32.

Workloads are limited to V32 (d_qk=576), s_q=1, h_q=32, no extra_kv, no
topk_length, no attn_sink (matches the kernel's hard asserts).

Sweep mirrors test_sm90_tp4_correctness.py for direct comparison:
  - b in {2, 16, 64}
  - s_kv in {2048, 8192}
...
def gen_h32_cases() -> List[RawTestParam]:
        ...
        for s_kv in [2048, 8192]:
            for topk in [256, 1024, 2048]:
                ...
                cases.append(RawTestParam(
                    b=b, h_q=32, s_q=1, h_kv=1, s_kv=s_kv,
...
def main():
    ...
    raw_cases = gen_h32_cases()
    print(f"{kk.colors['CYAN_BG']}{len(raw_cases)} h_q=32 testcases"
          f"{kk.colors['CLEAR']}")
